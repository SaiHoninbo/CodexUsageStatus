...
struct CodexUsageStatusTests {
    ...
    private static func testUsagePopoverTabsAndAppVersion() throws {
        try expect(
            ...
            "usage popover exposes Overview, History, Accounts, and Settings in primary navigation"
        )
        try expect(UsagePopoverTab.accounts.title == "帳號", "account management has a primary destination")
    ...
    private static func testReleaseArtifactValidation() throws {
        ...
        let artifactURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent("outputs/CodexUsageStatus.app.zip")
        let expectedArtifactVersion = "2.4.114"
