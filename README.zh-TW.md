...
請到「系統設定 → 隱私權與安全性 → 輔助使用」，啟用 `CodexUsageStatus.app`。
由於正式版本使用 ad-hoc 簽章，macOS 可能把覆蓋更新視為新的 App 身分而再次要求
授權；只有 App 明確提示權限失效時才需要重新啟用。
...
每個正式 artifact 的 checksum 與 ad-hoc 簽章驗證結果應記錄在 Release notes 或維護
evidence。只把 ZIP 提交到 `main` 並不會自動建立 App 內的 Release 更新。

上傳 ZIP 前，必須驗證最後要發布的那一份 artifact。驗證器會 fail closed，
只有在 archive 只包含預期 App、bundle identifier 與 semantic version 正確，並
通過 strict bundle verification 時才允許通過。本機、候選包與正式發布都使用同一套
ad-hoc 驗證：

```bash
./script/validate_release_artifact.sh outputs/CodexUsageStatus.app.zip 2.4.111
# 正式 GitHub Release 驗證：
./script/validate_release_artifact.sh --public-release outputs/CodexUsageStatus.app.zip 2.4.111
```
...
```bash
./script/validate_release_artifact.sh --public-release-developer-id outputs/CodexUsageStatus.app.zip 2.4.111 ABCDE12345
```
...
`package` mode 預設建立本機 ad-hoc artifact。正式 GitHub Release 設定
